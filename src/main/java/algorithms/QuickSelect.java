...
public class Quickselect {
  ...
  public int selectIterative(int[] nums, int start, int end, int k) {
    // k is 1-indexed
    ...
    if (left - start >= k)
      return selectIterative(nums, start, left - 1, k);
    else if (left - start + 1 == k && left == right)
      return nums[left];
    else
      return selectIterative(nums, right + 1, end, k - right + start - 1);
  }
